    ...
    return (
        <AdminLayout title="Languages & translations" actions={
            <div style={{ display: 'flex', gap: 10 }}>
                <Button size="sm" variant="secondary" onClick={() => post(route('admin.languages.sync'))}>Sync source strings</Button>
                <Button size="sm" onClick={() => post(route('admin.languages.translate'))}>Auto-translate missing</Button>
            </div>
        }>
                ...
                {languages.map((l) => {
                    ...
                    return (
                        <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '13px 18px', borderBottom: '1px solid var(--border-subtle)' }}>
                            ...
                            {l.is_default ? <Badge tone="accent">source</Badge> : (
                                l.enabled ? <Badge tone="active" dot>enabled</Badge> : <Badge tone="neutral">disabled</Badge>
                            )}
                            ...
                            {! l.is_default ? (
                                <>
                                    <Button size="sm" variant="secondary" onClick={() => post(route('admin.languages.translate'), { code: l.code })}>Translate</Button>
                                    <Link href={route('admin.translations.index', { locale: l.code })}><Button size="sm" variant="ghost">Edit</Button></Link>
                                    <Button size="sm" variant="ghost" onClick={() => post(route('admin.languages.update', l.id) /* toggle */, { enabled: ! l.enabled })}>{l.enabled ? 'Disable' : 'Enable'}</Button>
