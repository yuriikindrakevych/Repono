...
const Card = ({ title, children }) => (
    <div className="r-hairline" style={{ background: 'var(--surface-card)', borderRadius: 'var(--radius-lg)', overflow: 'clip', marginBottom: 22 }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-subtle)' }}><h2 style={{ fontSize: 'var(--fs-title)' }}>{t(title)}</h2></div>
...
export default function Show({ customer, subscriptions, licenses }) {
    ...
    return (
        <AdminLayout title={customer.name} actions={<Link href={route('admin.customers.index')}><Button size="sm" variant="secondary">← {t('Customers')}</Button></Link>}>
            ...
            <Card title="Subscriptions">
                {subscriptions.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>{t('None.')}</p> : subscriptions.map((s) => (
                    <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                        ...
                        <Button size="sm" variant="secondary" onClick={() => post('admin.subscriptions.extend', s.id)}>{t('Extend')}</Button>
                        <Button size="sm" variant="secondary" onClick={() => post('admin.subscriptions.reactivate', s.id)}>{t('Reactivate')}</Button>
                        <Button size="sm" variant="danger" onClick={() => post('admin.subscriptions.suspend', s.id, t('Suspend this subscription?'))}>{t('Suspend')}</Button>
                    </div>
                ))}
            </Card>

            <Card title="Licenses & activations">
                {licenses.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>{t('None.')}</p> : licenses.map((l) => (
                    <div key={l.id} style={{ padding: '12px 0', borderBottom: '1px solid var(--border-subtle)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                            ...
                            <Button size="sm" variant="secondary" onClick={() => post('admin.licenses.regenerate-key', l.id, t('Reissue the license key?'))}>{t('Reissue key')}</Button>
                            <Button size="sm" variant="secondary" onClick={() => post('admin.licenses.regenerate-token', l.id, t('Revoke and reissue the repo token?'))}>{t('Revoke token')}</Button>
                        </div>
                            ...
                            <div style={{ marginTop: 8, paddingLeft: 4, display: 'grid', gap: 6 }}>
                                {l.activations.map((a) => (
                                    <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                        ...
                                        <Button size="sm" variant="ghost" onClick={() => del('admin.activations.destroy', a.id, t('Reset activation :domain?', { domain: a.domain }))}>{t('Reset')}</Button>
                                    </div>
                                ))}
                            </div>
                        ) : null}
